...
const getByAdminValidator = (): ValidationChain[] => {
  return [
    query("page")
      .optional()
      .isInt({ min: 1 })
    ...
    query("sort")
      ...
      .custom((value) => {
        ...
        const sortColumn = Object.keys(Blog.getAttributes()).includes(value);
        if (!sortColumn) {
    ...
    query("contentLength")
      .optional()
      .isInt({ min: 1 })
      .withMessage("Content length must be a positive integer")
      .bail(),
    query("categorySlug")
      .optional()
      .isInt({ min: 1 })
...
export default getByAdminValidator;
